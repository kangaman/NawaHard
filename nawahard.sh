#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  NawaHard — Linux VPS Security Audit & Hardening Recommendation         ║
# ║  Version: 1.0.0                                                        ║
# ║  License: MIT                                                          ║
# ║  Repository: https://github.com/kangaman/NawaHard                      ║
# ╚══════════════════════════════════════════════════════════════════════════╝
#
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "NawaHard v${VERSION} — Linux VPS Security Audit"
            echo ""
            echo "Options:"
...
<title>NawaHard — Security Audit Report</title>
<style>
...
HTMLHEAD
    ...
    # Header
    cat >> "$REPORT_HTML" <<EOF
<h1>🛡️ NawaHard Security Audit</h1>
...
EOF
    ...
    echo "<footer>NawaHard v${VERSION} — Generated $(date) — https://github.com/kangaman/NawaHard</footer>" >> "$REPORT_HTML"
...
generate_txt() {
    cat > "$REPORT_TXT" <<EOF
╔══════════════════════════════════════════════════════════════╗
║  NawaHard — Security Audit Report                            ║
║  Version: ${VERSION}                                          ║
╚══════════════════════════════════════════════════════════════╝
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  NawaHard v${VERSION} — https://github.com/kangaman/NawaHard
EOF
}
...
main() {
    ...
    if [[ "$OPT_NOTIFY" -eq 1 ]] && [[ -n "${NAWAHARD_WEBHOOK:-}" ]]; then
        curl -s -X POST "$NAWAHARD_WEBHOOK" -H "Content-Type: application/json" \
            -d "{\"text\":\"🛡️ NawaHard: ${HOSTNAME} Score ${SCORE}/100 (P:${PASS} W:${WARN} F:${FAIL})\"}" &>/dev/null
    fi
